...
public class ProductRepository {
    ...
    public ProductRepository() {
        store.put("1", new Product("1", "iPhone 15 Pro (กิตติธัช คำโท 673380029-0 SEC 1)",
    ...
    public Flux<Product> findAll() {
        Collection<Product> product = store.values();
        return Flux.fromIterable(product)  ;
    ...
    public Flux<Product> findByCategory(String category) {
        // TODO: เติม code ตรงนี้\
        Flux<Product> product = findAll().filter(p -> p.getCategory().equalsIgnoreCase(category));
        return product; // ← แก้บรรทัดนี้
    }
}
